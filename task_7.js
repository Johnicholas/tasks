...
function Task(print) {
    ...
    function Spawn(address, priority, task, rest) {
	this.address = address
	...
	visitor.spawn(this.address, this.priority, this.task)
	this.rest.accept(visitor)
    }
    ...
    CommandPrinter.prototype.spawn = function (address, priority, task) {
	print('spawn '+address+' '+priority+' '+task)
    }
    ...
    RecordSchedulerCommand.prototype.spawn = function (address, priority, task) {
	this.command = new Spawn(address, priority, task, this.command)
    }
	...
	if (this.agenda.length > 0) {
	    ...
	    if (this.task_queues[this.current_event.task].length > 0) {
		message = this.task_queues[this.current_event.task].shift()
		command = this.tasks[this.current_event.task].step(new RecordScheduler(), message)
	    } else {
		command = this.tasks[this.current_event.task].step(new RecordScheduler())
	    }
            // print it
	    print(this.current_event.task+':')
	...
	this.agenda.push({
	    time: this.clock,
	    priority: this.task_priorities[this.current_event.task],
	    task: this.current_event.task
	})
    }
	...
	this.task_queues[address].push(message)
	this.agenda.push({
	    time: this.clock,
	    priority: this.task_priorities[address],
	    task: address
	})
    }
    ...
    Scheduler.prototype.spawn = function (task_name, priority, task) {
	if (this.tasks[task_name]) {
            throw 'cannot spawn a task on top of another task!'
	} else {
            this.tasks[task_name] = task
	    this.task_priorities[task_name] = priority
	    this.task_queues[task_name] = []
	    // TODO: should this be here?
            // this.agenda.push({time: this.clock, priority: priority, task: task})
	}
    }
